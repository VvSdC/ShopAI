...
function triggerLabel(selected) {
  if (!selected.length) return 'All brands'
  if (selected.length === 1) return selected[0]
  return `${selected.length} brands selected`
}

export default function BrandFilterModal({ brands = [], value = [], onChange }) {
                  ...
                  {filteredBrands.length === 0 ? (
                    ...
                    <ul className="grid grid-cols-1 gap-x-8 sm:grid-cols-2">
                      {filteredBrands.map((b) => {
                        ...
                        const inputId = `brand-filter-${b?._id || name}`
                        return (
                          <li key={b?._id || name}>
                            <label
                              htmlFor={inputId}
